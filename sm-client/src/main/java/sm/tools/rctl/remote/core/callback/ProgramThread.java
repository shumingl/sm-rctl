...
public class ProgramThread extends Thread {
    ...
    public void run() {
        try {
            ...
            Header header = new Header(config.getString("id"), session, RctlActions.CLIENT_SESSION);
            Message<RespMsg> response = channel.send(new Message<>(header, establish), RespMsg.class);
            RespMsg returnBody = response.getBody();
            logger.info(response.getBody().getMessage());
            // 等待和客户端输入
            if (returnBody.getResult() == RespMsg.RESULT.SUCCEED) {
